...
| --- | --- | --- | --- |
| Pocket Assistant | The iPhone-only `Pocket助手` target reuses this table's BLE → SoftAP → UDP/DUML, camera SET, gimbal, and safety behavior. It must not fork protocol tables or HeadTrack control. | This focused product has Chinese `头追 / 云台 / 拍摄 / 设备` navigation and no monitor/media surface. Android parity is outside the initial extraction. | **physical iOS** |
