...
| Assists | Toolbar 1:1 (LUT, PEAK, FALSE, ZEBRA, WAVE, PARADE, HISTO, VECTOR, LIGHTS, AUDIO, GUIDES, GRID, CROSS, MIRROR); long-press options; WAVE hold-without-drag opens options; scope plate metrics (`ScopeMiniChrome`); number fields in those options (Zebra Highlight / Midtone) lift above the keyboard; number-pad Done dismisses the pad (tap outside still dismisses the popup) | Metal vs Vulkan vs GLES; Vision vs `android.media.FaceDetector`; PixelCopy / Kyant sampling | **physical** both |
...
| Head tracking | iOS: Controls **Head Tracking (Experimental)** (off by default). AirPods with motion. **HeadTrack Control** is a video-free control cover with AirPods/Pocket/Datalink/Gimbal status, live head/gimbal/target yaw-pitch, gimbal recenter before calibration, Apple's supported-headphone help, Fast/Standard/Gentle presets, and Sensitivity/Dead Zone/Smoothness/Max Speed. **Calibrate Head Lock** is shared forward — that head pose and that gimbal pose are zero. The App-level motion bridge keeps driving when the live view is covered. Stick throws through the existing 25 Hz path until live `0x04/0x05` matches the configured target; Pocket has no angle SET. Gimbal Fast + tilt unlocked at calibrate. Roll is shown, not driven. STOP clears SET. AirPods/Pocket/datalink loss, 250 ms motion timeout, and app inactive/background immediately rest the stick and clear SET. Chip/stick/gamepad win while thrown. Toast if IMU is missing or the head is moving at calibrate. | Android has no AirPods IMU — no Controls row | **physical** iOS |
