...
- **Personal Team development builds join manually.** Apple does not provision Hotspot Configuration for a free Personal Team. When iOS reports that automatic configuration is unavailable, the wizard shows the target SSID and waits for the operator to join it in Settings → Wi-Fi. Returning to the app detects the camera's `192.168.2.x` path and continues without restarting pairing. Paid-team and distribution builds keep the automatic join flow.
